...
const FacilityMap: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedFacility !== null && cardContainerRef.current && mapRef.current) {
      const card = cardContainerRef.current.children[selectedFacility] as HTMLElement;
      if (card) {
        cardContainerRef.current.scrollTo({
          top: card.offsetTop - cardContainerRef.current.offsetTop,
          behavior: "smooth",
        });
      }
    }
  ...
  useEffect(() => {
    if (selectedFacility !== null && mapRef.current && markersRef.current.length > 0) {
  ...
  const handleDirectionButtonClick = (index: number) => {
    if (clientLocation && facilityData[index]) {
      getDirections(clientLocation, [facilityData[index].lon, facilityData[index].lat]);
    }
  };
      ...
      {clientLocation ? (
        <>
          <div
            ...
            className="facility-container flex flex-col md:w-1/3 m-4 shadow-lg overflow-y-auto"
          >
            {facilityData.map((info, index) => (
              <div
                key={index}
                className={`facility-card p-4 bg-white rounded-md border border-gray-300 cursor-pointer mb-4 
                ${selectedFacility === index ? "selected bg-green-100" : ""}`}
                onClick={() => {
                  setSelectedFacility(index);
                }}
              >
                <div className="facility-card-header flex justify-between items-center mb-2">
                  <h2 className="facility-card-title text-xl font-semibold">{info.name}</h2>
                  {info.verified ? (
                ...
                <div className="facility-card-body">
                  ...
                  <p className="text-gray-600 pb-2">Distance: {info.distance.toFixed(2)} km away</p>
                </div>
                <div className="facility-card-actions flex space-x-6">
                  <button 
                    className="btn-md btn-primary"
                    onClick={(e) => {
                      e.stopPropagation();
                      handleDirectionButtonClick(index);
                    }}
                  >
                    Get Directions
                  </button>
